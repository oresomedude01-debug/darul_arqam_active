...
<div class="bg-gray-50 min-h-screen">
    ...
    <div class="bg-white border-b border-gray-200">
        <div class="max-w-3xl mx-auto px-4 py-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Edit Fee Item</h1>
                    <p class="mt-1 text-sm text-gray-600">Update fee item details</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-3xl mx-auto px-4 py-8">
        <div class="bg-white rounded-lg shadow p-8">
            <form action="{{ route('billing.fee-items.update', $feeItem) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" 
                           ...
                           value="{{ old('name', $feeItem->name) }}" required>
                    @error('name')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                              ...
                              placeholder="Describe what this fee covers">{{ old('description', $feeItem->description) }}</textarea>
                    @error('description')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="is_optional" name="is_optional"
                        ...
                        <option value="0" {{ old('is_optional', $feeItem->is_optional ? '1' : '0') === '0' ? 'selected' : '' }}>Mandatory</option>
                        <option value="1" {{ old('is_optional', $feeItem->is_optional ? '1' : '0') === '1' ? 'selected' : '' }}>Optional</option>
                    </select>
                    @error('is_optional')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="default_amount" name="default_amount" 
                               ...
                               value="{{ old('default_amount', $feeItem->default_amount) }}">
                    </div>
                ...
                <div>
                    ...
                    <select id="status" name="status"
                        ...
                        <option value="active" {{ old('status', $feeItem->status) === 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $feeItem->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center gap-4 pt-6 border-t border-gray-200">
                    <button type="submit" 
                        ...
                        <i class="fas fa-save mr-2"></i>Update Fee Item
                    </button>
